...
yReal  = tf.transpose(y, perm=[2,0,1])
yPred = tf.transpose(predicTotal_0, perm=[2,0,1])
cost = tf.losses.mean_squared_error(yReal,yPred, reduction=tf.losses.Reduction.NONE)#, weights = (1/n_classes)*np.ones((n_classes,))
cost = tf.reduce_mean(tf.reduce_mean(cost, axis=1), axis=1)
...
train_op = tf.train.AdamOptimizer(lr, epsilon=0.1).minimize(tf.reduce_sum(cost)+regularization_penalty*100000000)
