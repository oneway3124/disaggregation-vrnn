...
class UKdale(TemporalSeries, SequentialPrepMixin): #TemporalSeries comes from class Data (in cle/cle/data/init) and it has ITERATOR
    """
    ...
    def theano_valTime_vars(self):
        return [T.bscalar('valTime')]
    ...
    def slices(self, start, end):
        ...
        if self.cond:
            return totuple([batches[0], mask, label_batches[0], label_mask])#, self.validTime
        else:
