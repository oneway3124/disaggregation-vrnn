    ...
    def load_csvdata(self, data_path, numApp, typeLoad=0, num_sequences_per_batch=0, target_inclusion_prob=0.5):#appliances, filename, self.sample_period, windows
        ...
        if(typeLoad==1):
            ...
            for building_i, window in self.windows['train'].items(): #self.windows.items()
                ...
                if (numApp!=-1):
                    truFileName=data_path+"/pickles/"+str(building_i)+'_'+self.listAppliances[numApp]+'_'+str(self.sample_period)+'_'+str(num_sequences_per_batch)+'_'+window[0]+'_'+window[1]#fileName[pos:]
                else:
                    truFileName=data_path+"/pickles/"+str(building_i)+'_'+'all'+'_'+str(self.sample_period)+'_'+str(num_sequences_per_batch)+'_'+window[0]+'_'+window[1]
            ...
            for building_i, window in self.windows.items():
                if (numApp!=-1):
                    truFileName=data_path+"/pickles/"+str(building_i)+'_'+self.listAppliances[numApp]+'_'+str(self.stride_input)+'_'+window[0]+'_'+window[1]#fileName[pos:]
                else:
                    truFileName=data_path+"/pickles/"+str(building_i)+'_'+'all'+'_'+str(self.stride_input)+'_'+window[0]+'_'+window[1]
