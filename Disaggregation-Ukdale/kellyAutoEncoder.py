...
windows = {1:("2013-02-27", "2014-02-27")}
...
def main(args):
    ...
    XdataSet,YdataSet = reader.load_csvdata(period, data_path, target_inclusion_prob, nilmkt_fileName, applianceTest, loadAsKelly, num_sequences_per_batch) 
    ...
    y_val = y_val.reshape((-1, n_steps, 1))
    x_test = x_test.reshape((-1, n_steps, 1))
    y_test = y_test.reshape((-1, n_steps, 1))
    ##############   STARTING TO CONSTRUCT GRAPH: INPUTS AND OPERATIONS #####################

    x = tf.placeholder(tf.float32, [None, n_steps, 1]) #batch_size x length x dimensions
    ...
    layer_2_units = (n_steps-3)*8

    #layer 1
    conv1 = tf.layers.conv1d(inputs=x, filters=8, kernel_size=4, strides=1,
        padding='valid', activation = tf.nn.relu)
    conv1 = tf.reshape(conv1,[-1, layer_2_units])

    #layer 2
    Win_2 = weight_variable([layer_2_units,128])
    bin_2 = bias_variable([128])
    out_2 = tf.nn.relu(tf.matmul(conv1, Win_2) +  bin_2)

    #layer 3
    Win_3 = weight_variable([128,layer_2_units])
    bin_3 = bias_variable([layer_2_units])
    out_3 = tf.nn.relu(tf.matmul(out_2, Win_3) +  bin_3)

    #layer 4
    Win_4 = weight_variable([layer_2_units,n_steps])
    bin_4 = bias_variable([n_steps])
    out_4 = tf.nn.relu(tf.matmul(out_3, Win_4) +  bin_4)
    out_4 = tf.expand_dims(out_4, 2)

    #layer 5
    pred = tf.layers.conv1d(inputs=out_4, filters=1, kernel_size=4, strides=1,
        padding='same', activation = tf.nn.relu) #for now use same, the paper says valid

    #pred has too many predictions
    ...
    tf.summary.histogram('Win_3', Win_3)
    tf.summary.histogram('Win_4', Win_4)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        config_file_name = 'config_kelly.txt'
