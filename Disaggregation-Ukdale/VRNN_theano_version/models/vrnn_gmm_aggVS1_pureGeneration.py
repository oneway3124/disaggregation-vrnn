...
def main(args):
    ...
    ((s_temp_val, prediction_val, z_t_temp_val, theta_1_temp_val, theta_mu_temp_val, theta_sig_temp_val, coeff_temp_val), updates_val) =\
        theano.scan(fn=inner_val_fn , n_steps=n_steps-1,
    ...
    ((s_temp, phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp,z_t_temp, z_1_temp, theta_1_temp, theta_mu_temp, theta_sig_temp, coeff_temp, prediction), updates) =\
        theano.scan(fn=inner_train_fn, sequences=[x[1:,:]],#[x_1_temp],
    ...
    mse = T.mean((prediction - x[1:,:])**2) # As axis = None is calculated for all
    mae = T.mean( T.abs_(prediction - x[1:,:]) )
    mse.name = 'mse'
    mae.name = 'mae'
    pred_in = x[:-1,:].reshape((batch_size*(n_steps-1),-1))

    kl_temp = KLGaussianGaussian(phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp)

    target_shape = x[:,1:].shape
    theta_mu_in = theta_mu_temp.reshape((batch_size*(n_steps-1), -1))
    theta_sig_in = theta_sig_temp.reshape((batch_size*(n_steps-1), -1))
    coeff_in = coeff_temp.reshape((batch_size*(n_steps-1), -1))

    recon = GMM(pred_in, theta_mu_in, theta_sig_in, coeff_in)# BiGMM(x_in, theta_mu_in, theta_sig_in, coeff_in, corr_in, binary_in)
    recon = recon.reshape((batch_size,n_steps-1))
    recon.name = 'gmm_out'
    ...
    mse_val = T.mean((prediction_val - x[1:,:])**2) # As axis = None is calculated for all
    mae_val = T.mean( T.abs_(prediction_val - x[1:,:]) )
    mse_val.name = 'mse_val'
    mae_val.name = 'mae_val'
    pred_in_val = x[:-1,:].reshape((batch_size*(n_steps-1), -1))
    ...
    theta_mu_in_val = theta_mu_temp_val.reshape((batch_size*(n_steps-1), -1))
    theta_sig_in_val = theta_sig_temp_val.reshape((batch_size*(n_steps-1), -1))
    coeff_in_val = coeff_temp_val.reshape((batch_size*(n_steps-1), -1))
    ...
    recon_val = recon_val.reshape((batch_size,n_steps-1))
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq,
                   ddout=[recon_term_val, mse_val, mae_val, prediction_val, theta_mu_temp_val],
                   indexSep=3,
                   indexDDoutPlot = [(0,prediction_val)],
                   instancesPlot = instancesPlot, #[0,40],#, 80,150
    ...
    lr_iterations = {0:lr,20:(lr/10),120:(lr/100)}

    mainloop = Training(
        name=pkl_name,
        data=Iterator(train_data, batch_size),#shuffle false by default
        model=model,
        optimizer=optimizer,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        config_file_name = 'config_pure.txt'
    ...
    params['save_path'] = params['save_path']+'/gmm_pure/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")+'_app'+params['flgAgg']
    os.makedirs(params['save_path'])
    shutil.copy('config_pure.txt', params['save_path']+'/config_pure.txt')
