...
def main(args):
    ...
    save_path = args['save_path']#+'/aggVSdisag/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
    ...
    def inner_fn(x_t, s_tm1):
        ...
        '''
        x_pred = GMM_sample(theta_mu_t, theta_sig_t, coeff_t) #Gaussian_sample(theta_mu_t, theta_sig_t)
        s_t = rnn.fprop([[x_t, z_1_t], [s_tm1]], params)
        y_pred = dissag_pred.fprop([s_t], params)

        return s_t, phi_mu_t, phi_sig_t, prior_mu_t, prior_sig_t, z_t,  z_1_t, theta_1_t, theta_mu_t, theta_sig_t, coeff_t, x_pred, y_pred
        #corr_temp, binary_temp
    ((s_temp, phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp,z_t_temp, z_1_temp, theta_1_temp, theta_mu_temp, theta_sig_temp, coeff_temp, x_pred_temp, y_pred_temp), updates) =\
        theano.scan(fn=inner_fn,
                    sequences=[x_1_temp],
                    outputs_info=[s_0, None, None, None, None, None, None,  None, None, None, None, None, None])
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq,
                   ddout=[nll_upper_bound, recon_term, kl_term, mse,
                          ...
                          coeff_max, coeff_min, coeff_mean_max, coeff_mean_min,#23
                          theta_mu_temp, theta_sig_temp, z_t_temp, x_pred_temp,y_pred_temp,#corr_temp, binary_temp, 
                          coeff_temp, #22
                          s_temp, z_1_temp],
                   indexSep=23,
                   indexDDoutPlot = [(0,theta_mu_temp), (2, z_t_temp), (3,x_pred_temp),(4,y_pred_temp)], # adding indexes of ddout for the plotting
                   instancesPlot = [0, 150],
...
if __name__ == "__main__":
    ...
    params['save_path'] = params['save_path']+'/aggVSdisag/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
