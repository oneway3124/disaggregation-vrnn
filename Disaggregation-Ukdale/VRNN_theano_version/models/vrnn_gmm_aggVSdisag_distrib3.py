...
from cle.cle.cost import BiGMM, KLGaussianGaussian, GMM,GMMdisag3
...
def main(args):
    ...
    nodes = [rnn,
             ...
             theta_1, theta_mu1, theta_mu2, theta_mu3, theta_sig1, theta_sig2, theta_sig3, coeff1, coeff2 ,coeff3]
    ...
    def inner_fn(x_t, s_tm1):
        ...
        return s_t, phi_mu_t, phi_sig_t, prior_mu_t, prior_sig_t, z_t,  z_1_t, theta_1_t, theta_mu1_t, theta_sig1_t, coeff1_t, theta_mu2_t, theta_sig2_t, coeff2_t, theta_mu3_t, theta_sig3_t, coeff3_t,y_pred1, y_pred2, y_pred3
        #corr_temp, binary_temp
    ((s_temp, phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp,z_t_temp, z_1_temp, theta_1_temp, theta_mu1_temp, theta_sig1_temp, coeff1_temp, theta_mu2_temp, theta_sig2_temp, coeff2_temp, theta_mu3_temp, theta_sig3_temp, coeff3_temp,  y_pred1_temp, y_pred2_temp, y_pred3_temp), updates) =\
        theano.scan(fn=inner_fn,
                    sequences=[x_1_temp],
                    outputs_info=[s_0,  None, None, None, None, None, None, None, None,None,  None, None, None, None, None, None, None, None, None, None])
    ...
    z_t_temp.name = 'z'
    theta_1_temp.name = 'theta_1_temp'
    ...
    y_pred_temp = T.stack([y_pred1_temp, y_pred2_temp, y_pred3_temp], axis=2) 
    ...
    # y_pred_temp now steps, batch, apps
    # y is batch, steps, apps
    #y_aux = y.reshape((y.shape[0], y.shape[1],-1))
    
    mse = T.mean((y_pred_temp - y)**2) # CHECK RESHAPE with an assertion
    mse1 = T.mean((y_pred_temp[:,:,0] - y[:,:,0])**2)
    mse2 = T.mean((y_pred_temp[:,:,1] - y[:,:,1])**2)
    mse3 = T.mean((y_pred_temp[:,:,2] - y[:,:,2])**2)  
    mseTotal =  (1/3)*(mse1+mse2+mse3)
    ...
    recon = GMMdisag3(y_in, theta_mu1_in, theta_sig1_in, coeff1_in,
                      theta_mu2_in, theta_sig2_in, coeff2_in,
                      theta_mu3_in, theta_sig3_in, coeff3_in)# BiGMM(x_in, theta_mu_in, theta_sig_in, coeff_in, corr_in, binary_in)
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq,
                   ddout=[nll_upper_bound, recon_term, kl_term, mse, mse1, mse2, mse3, mseTotal,#2
                          theta_1_temp,  theta_mu1_temp, theta_sig1_temp,theta_mu2_temp, 
                          theta_sig2_temp,theta_mu3_temp, theta_sig3_temp,
                          coeff1_temp,coeff2_temp,coeff3_temp, 
                          s_temp, z_t_temp, z_1_temp,y_pred_temp], # y_pred always at last for an index matter when plotting
                   indexSep=8,
                   indexDDoutPlot = [(0,theta_1_temp),(11, z_t_temp),(13, y_pred_temp)], # adding indexes of ddout for the plotting
                   #, (6,y_pred_temp)
                   instancesPlot = [],#0-150
    ...
    fLog.write("log,kl,nll_upper_bound,mse, mse1, mse2, mse3, mseTotal\n")
    for i , item in enumerate(mainloop.trainlog.monitor['nll_upper_bound']):
      ...
      mse_m = mainloop.trainlog.monitor['mseX'][i]
      fLog.write("{},{},{},{}\n".format(a,b,d,mse_m ))
    fLog.close() 
...
if __name__ == "__main__":
    ...
    params['save_path'] = params['save_path']+'/aggVSdisag_distrib3/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
