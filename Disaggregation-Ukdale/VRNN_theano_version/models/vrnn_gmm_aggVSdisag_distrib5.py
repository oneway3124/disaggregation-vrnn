...
from cle.cle.cost import BiGMM, KLGaussianGaussian, GMMdisag5
...
windows = {1:("2013-02-01", "2017-02-01")}#, 2:("2013-02-27", "2013-04-27")
...
def main(args):
    ...
    model = Model()
    Xtrain, ytrain, Xval, yval, reader = fetch_ukdale(data_path, windows, appliances,numApps=-1, period=period,n_steps= n_steps, 
    ...
    nodes = [rnn,
             ...
             theta_1, theta_mu1, theta_mu2, theta_mu3, theta_mu4, theta_mu5,
             theta_sig1, theta_sig2, theta_sig3, theta_sig4, theta_sig5,
    ...
    def inner_fn(x_t, s_tm1):
        ...
        return (s_t, phi_mu_t, phi_sig_t, prior_mu_t, prior_sig_t, z_t,  z_1_t, theta_1_t, 
                  ...
                  theta_mu5_t, theta_sig5_t, coeff5_t, y_pred1, y_pred2, y_pred3, y_pred4, y_pred5)
    ...
    mse1 = T.mean((y_pred1_temp - y_input[:,:,0].reshape((y_input.shape[0],y_input.shape[1],1)))**2) # As axis = None is calculated for all
    mae1 = T.mean( T.abs_(y_pred1_temp - y_input[:,:,0].reshape((y_input.shape[0],y_input.shape[1],1))) )
    mse1.name = 'mse1'
    mae1.name = 'mae1'

    mse2 = T.mean((y_pred2_temp - y_input[:,:,1].reshape((y_input.shape[0],y_input.shape[1],1)))**2) # As axis = None is calculated for all
    mae2 = T.mean( T.abs_(y_pred2_temp - y_input[:,:,1].reshape((y_input.shape[0],y_input.shape[1],1))) )
    mse2.name = 'mse2'
    mae2.name = 'mae2'

    mse3 = T.mean((y_pred3_temp - y_input[:,:,2].reshape((y_input.shape[0],y_input.shape[1],1)))**2) # As axis = None is calculated for all
    mae3 = T.mean( T.abs_(y_pred3_temp - y_input[:,:,2].reshape((y_input.shape[0],y_input.shape[1],1))) )
    mse3.name = 'mse3'
    mae3.name = 'mae3'

    mse4 = T.mean((y_pred4_temp - y_input[:,:,3].reshape((y_input.shape[0],y_input.shape[1],1)))**2) # As axis = None is calculated for all
    mae4 = T.mean( T.abs_(y_pred4_temp - y_input[:,:,3].reshape((y_input.shape[0],y_input.shape[1],1))) )
    mse4.name = 'mse4'
    mae4.name = 'mae4'

    mse5 = T.mean((y_pred5_temp - y_input[:,:,4].reshape((y_input.shape[0],y_input.shape[1],1)))**2) # As axis = None is calculated for all
    mae5 = T.mean( T.abs_(y_pred5_temp - y_input[:,:,4].reshape((y_input.shape[0],y_input.shape[1],1))) )
    mse5.name = 'mse5'
    mae5.name = 'mae5'
    ...
    recon = GMMdisag5(y_in, theta_mu1_in, theta_sig1_in, coeff1_in,
                      theta_mu2_in, theta_sig2_in, coeff2_in,
                      theta_mu3_in, theta_sig3_in, coeff3_in,
                      theta_mu4_in, theta_sig4_in, coeff4_in,
                      theta_mu5_in, theta_sig5_in, coeff5_in)# BiGMM(x_in, theta_mu_in, theta_sig_in, coeff_in, corr_in, binary_in)
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq,
                   ddout=[nll_upper_bound, recon_term, kl_term, 
                          ...
                          y_pred1_temp, y_pred2_temp, y_pred3_temp, y_pred4_temp, y_pred5_temp ],
                   indexSep=13,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        config_file_name = 'config_all.txt'
    ...
    params['save_path'] = params['save_path']+'/aggVSdisag_distrib5/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
    os.makedirs(params['save_path'])
    shutil.copy('config_all.txt', params['save_path']+'/config.txt')
