...
def main(args):
    ...
    save_path = args['save_path']#+'/aggVSdisag_distribEach/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
    ...
    def inner_fn(x_t, s_tm1):
        ...
        '''
        y_pred = GMM_sampleY(theta_mu_t, theta_sig_t, coeff_t) #Gaussian_sample(theta_mu_t, theta_sig_t)
    ...
    recon = GMM(y_in, theta_mu_in, theta_sig_in, coeff_in)# BiGMM(x_in, theta_mu_in, theta_sig_in, coeff_in, corr_in, binary_in)
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq, 
                   ddout=[nll_upper_bound, recon_term, kl_term, #mse,
                          ...
                          theta_mu_temp, theta_sig_temp, z_t_temp,y_pred_temp,#corr_temp, binary_temp, 
                          coeff_temp, #22
                          s_temp, z_1_temp],
                   indexSep=22,
                   indexDDoutPlot = [(0,theta_mu_temp), (2, z_t_temp), (3,y_pred_temp)], # adding indexes of ddout for the plotting
                   instancesPlot = [0, 150],
...
if __name__ == "__main__":
    ...
    params['save_path'] = params['save_path']+'/aggVSdisag_distribEach/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
