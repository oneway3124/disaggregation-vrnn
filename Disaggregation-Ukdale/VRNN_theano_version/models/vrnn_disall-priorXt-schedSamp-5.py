...
windows = {1:("2014-08-12", "2015-04-01")}#, 2:("2013-02-27", "2013-04-27")
#2014-2017: 50000
...
def main(args):
    ...
    Xtrain, ytrain, Xval, yval, Xtest, ytest, reader = fetch_ukdale(data_path, windows, appliances,numApps=-1, period=period,n_steps= n_steps, 
                                              ...
                                              trainPer=0.5, valPer=0.25, testPer=0.25)
    ...
    nodes = [rnn,
             ...
             theta_mu2,theta_sig2,coeff2,
             theta_mu3,theta_sig3,coeff3,
             theta_mu4,theta_sig4,coeff4,
             theta_mu5,theta_sig5,coeff5]
    ...
    def inner_fn_test(x_t, s_tm1):
        ...
        pred_t=y_1.fprop(T.concatenate([y_pred1, y_pred2,y_pred3, y_pred4, y_pred5],axis=1), params)
        #y_pred = [GMM_sampleY(theta_mu_t[i], theta_sig_t[i], coeff_t[i]) for i in range(y_dim)]#T.stack([y_pred1,y_pred2],axis = 0 )
        s_t = rnn.fprop([[x_t, z_1_t, pred_t], [s_tm1]], params)
    ...
    def inner_fn(x_t, y_t, scheduleSamplingMask, s_tm1):
        ...
        if (scheduleSamplingMask==1):
          ...
          y_t_aux = y_1.fprop([T.concatenate([y_pred1, y_pred2,y_pred3, y_pred4, y_pred5],axis=1)], params)
        ...
        #corr_temp, binary_temp
    ((s_temp, phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp,  theta_mu1_temp, theta_sig1_temp, coeff1_temp, y_pred1_temp, theta_mu2_temp, theta_sig2_temp, coeff2_temp, y_pred2_temp, theta_mu3_temp, theta_sig3_temp, coeff3_temp, y_pred3_temp, theta_mu4_temp, theta_sig4_temp, coeff4_temp, y_pred4_temp, theta_mu5_temp, theta_sig5_temp, coeff5_temp, y_pred5_temp), updates) = theano.scan(fn=inner_fn, sequences=[x_1_temp, y_1_temp, scheduleSamplingMask],
                            outputs_info=[s_0, None, None, None, None, None,None,  None, None, None, None, None, None,None, None, None, None, None, None, None, None, None, None, None, None] )
    '''
    ((s_temp, phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp,z_temp, z_1_temp, theta_1_temp, 
    ...
    prediction_val = T.concatenate([y_pred1_temp_val,y_pred2_temp_val,y_pred3_temp_val,y_pred4_temp_val, y_pred5_temp_val], axis=2)

    recon_val = GMMdisagMulti(y_dim, y_in, theta_mu1_in_val, theta_sig1_in_val, coeff1_in_val, theta_mu2_in_val, theta_sig2_in_val, coeff2_in_val, theta_mu3_in_val, theta_sig3_in_val, coeff3_in_val, theta_mu4_in_val, theta_sig4_in_val, coeff4_in_val, theta_mu5_in_val, theta_sig5_in_val, coeff5_in_val)
    recon_val = recon_val.reshape((x_shape[0], x_shape[1]))
    recon_val.name = 'gmm_out'
    totaMSE_val=(mse1_val+mse2_val+mse3_val+mse4_val+mse5_val)/y_dim
    totaMAE_val=(mae1_val+mae2_val+mae3_val+mae4_val+mae5_val)/y_dim
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq,
                   ddout=[nll_upper_bound, recon_term, kl_term, mse1, mae1,mse2, mae2,mse3, mae3,mse4, mae4, mse5, mae5,
