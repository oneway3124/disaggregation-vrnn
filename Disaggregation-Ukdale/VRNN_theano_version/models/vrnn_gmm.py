...
from preprocessing.ukdale import UKdale
from VRNN_theano_version.ukdale_utils import fetch_ukdale
...
def main(args):
    ...
    x2s_dim = 7#250
    z2s_dim = 8#150
    target_dim = x_dim*k#x_dim #(x_dim-1)*k
    num_apps=3
    ...
    ((s_temp, phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp,z_t_temp, z_1_temp, theta_1_temp, theta_mu_temp, theta_sig_temp, coeff_temp, x_pred_temp), updates) =\
    ...
    mse = T.mean((x_pred_temp - x)**2) # CHECK RESHAPE with an assertion
    mse.name = 'mseX'
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq,
                   ddout=[nll_upper_bound, recon_term, kl_term,
                          ...
                          coeff_max, coeff_min, coeff_mean_max, coeff_mean_min,#21
                          theta_mu_temp, theta_sig_temp, z_t_temp, x_pred_temp,#corr_temp, binary_temp, 
                          coeff_temp, #22
                          s_temp, z_1_temp],
                   indexSep=22,
                   indexDDoutPlot = [(0,theta_mu_temp), (2, z_t_temp), (3,x_pred_temp)],
    ...
    for i , item in enumerate(mainloop.trainlog.monitor['nll_upper_bound']):
      fLog.write("{}\n".format(mainloop.trainlog.monitor['nll_upper_bound'][i]))
