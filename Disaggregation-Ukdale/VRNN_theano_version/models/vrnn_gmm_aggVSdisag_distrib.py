...
from cle.cle.cost import BiGMM, KLGaussianGaussian, GMM,GMMdisag
...
def main(args):
    ...
    for i in range(y_dim):
      theta_mu.append(FullyConnectedLayer(name='theta_mu'+str(i),
                                   parent=['theta_1'],
                                   parent_dim=[p_x_dim],
                                   nout=target_dim,
                                   unit='linear',
                                   init_W=init_W,
                                   init_b=init_b))
      theta_sig.append(FullyConnectedLayer(name='theta_sig'+str(i),
                                parent=['theta_1'],
                                parent_dim=[p_x_dim],
                                nout=target_dim,
                                unit='softplus',
                                ...
                                init_W=init_W,
                                init_b=init_b_sig))
      coeff.append(FullyConnectedLayer(name='coeff'+str(i),
                                  parent=['theta_1'],
                                  parent_dim=[p_x_dim],
                                  nout=k,
                                  unit='softmax',
    ...
    def inner_fn(x_t, s_tm1):
        ...
        theta_sig_t = TL.TypedListType(T.TensorType('float64', (False,)*2))()
...
#    ((s_temp, phi_mu_temp, phi_sig_temp, prior_mu_temp, prior_sig_temp,z_t_temp, z_1_temp, theta_1_temp, theta_mu_temp, theta_sig_temp, coeff_temp, y_pred_temp), updates) =\
    ...
    theta_mu_in = [theta_mu_temp[i].reshape((y_shape[0]*y_shape[1], -1)) for i in range(y_dim)]
    theta_sig_in = [theta_sig_temp[i].reshape((y_shape[0]*y_shape[1], -1)) for i in range(y_dim)]
    coeff_in = [coeff_temp[i].reshape((y_shape[0]*y_shape[1], -1)) for i in range(y_dim)]
    ...
    recon = GMMdisagMulti(y_in, y_dim,  theta_mu_in, theta_sig_in, coeff_in)# BiGMM(x_in, theta_mu_in, theta_sig_in, coeff_in, corr_in, binary_in)
    ...
    extension = [
        ...
        Monitoring(freq=monitoring_freq,
                   ddout=[nll_upper_bound, recon_term, kl_term,#2
                          theta_mu_temp, theta_sig_temp,
                          z_t_temp,#y_pred_temp, 
                          coeff_temp, s_temp, z_1_temp],
                   indexSep=1, # 0 for previous function, 1 for ploting personalized
                   indexDDoutPlot = [(3,theta_mu_temp), (5, z_t_temp)], # adding indexes of ddout for the plotting
                   #, (6,y_pred_temp)
...
if __name__ == "__main__":
    ...
    params['save_path'] = params['save_path']+'/aggVSdisag_distrib/'+datetime.datetime.now().strftime("%y-%m-%d_%H-%M")
