...
class RealAggregateSource(ActivationsSource):
    ...
    def _find_sections_with_no_target(self):
        ...
        for fold, sects_per_building in self.mains_good_sections.iteritems():
            for building, good_sections in sects_per_building.iteritems():
                ...
                #    print(activation.index[0],activation.index[-1])
                for activation in activations:
                    if (prev_end>=activation.index[0]):
                        continue
    ...
    def _compute_gap_probabilities_dave(self):
        # Choose a building and a gap
        self.all_gaps = {}
        #for fold in DATA_FOLD_NAMES:
        #    print(DATA_FOLD_NAMES)
        #    print(fold)
        fold = DATA_FOLD_NAMES
        all_gaps_for_fold = []
    ...
    def _get_sequence(self, fold='train', enable_all_appliances=False):
        if enable_all_appliances:
            ...
            #raise ValueError("`enable_all_appliances` is not implemented yet for RealAggregateSource!")
        ...
        for retry_i in range(MAX_RETRIES):
            seq = _seq_getter_func(fold=fold)#allAps=enable_all_appliances
            if seq is None:
                continue
